...
echo + "VERSION=\"\$(podman run -i --rm $(quote "$IMAGE_ID") pacman -Syi borg | sed -ne 's/^Version\s*: \(.*\)$/\1/p')\"" >&2
VERSION="$(podman run -i --rm "$IMAGE_ID" pacman -Syi borg | sed -ne 's/^Version\s*: \(.*\)$/\1/p')"

if [ -z "$VERSION" ]; then
    echo "Unable to read version of the 'borg' Pacman package: Package not found" >&2
    exit 1
elif ! [[ "$VERSION" =~ ^([0-9]+)\.([0-9]+)\.([0-9]+)([+~-]|$) ]]; then
    echo "Unable to read version of the 'borg' Pacman package: '$VERSION' is no valid version" >&2
    exit 1
fi
